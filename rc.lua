...
local scrlocker    = "i3lock -с 000000"
...
function connect(s)
  ...
  awful.tag({ "1", "2", "3", "4", "5", "6", "7", "8", "9", "a", "b", "c", "d" }, s, layout)
...
globalkeys = awful.util.table.join(
    -- X screen locker
    awful.key({ modkey, "Control" }, "l", function () awful.spawn(scrlocker) end,
    ...
    awful.key({  }, "XF86AudioRaiseVolume",
        function ()
            awful.util.spawn_with_shell("amixer -q -D pulse sset Master 5%+", false)
    ...
    awful.key({ modkey }, "x", function ()
        awful.spawn(string.format("d_run -i -fn 'Monospace' -nb '%s' -nf '%s' -sb '%s' -sf '%s'",
...
-- awful.spawn("firefox", {tag = screen[2].tags[1]})
-- naughty.notify({
--       title = "Title",
--       text = tostring(#screen[2].tags),
-- })
